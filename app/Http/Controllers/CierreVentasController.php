...
class CierreVentasController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Ingresos recibidos: sumar pagos (no valor_v) cuyas fechas de pago estén en el mes seleccionado
        // y que correspondan a ventas a crédito de meses anteriores
        $ingresosRecibidos = $this->getIngresosRecibidos($startOfMonth, $endOfMonth, $metodosPago);
            ...
            ->map(function ($tecnico) use ($startDate, $endDate, $metodosPago) {
                ...
                foreach ($tecnico->ventas as $venta) {
                    ...
                    foreach ($pagos as $pago) {
                        ...
                        // Solo considerar pagos recibidos dentro del rango seleccionado
                        if (!$fechaPago->between($startDate, $endDate)) {
                            continue;
                        }

                        // Incluir pagos de ventas de meses anteriores al pago (contado o crédito)
