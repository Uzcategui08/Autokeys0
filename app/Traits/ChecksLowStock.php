...
trait ChecksLowStock
{
     ...
     * @return void
     */
    protected function checkAndNotifyLowStock(Producto $producto, int $currentStock, int $threshold = 3): void
    {
        if ($currentStock <= $threshold && $currentStock > 0) {
            $users = \App\Models\User::role('admin')->get();

            foreach ($users as $user) {
                $user->notify(new LowStockNotification($producto, $currentStock));
            }
        }
    }
}
