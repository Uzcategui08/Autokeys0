...
<h3>Job</h3>
        @foreach($items as $itemGroup)
            @php
                $jobName = $itemGroup['job'] ?? $itemGroup['trabajo'] ?? $itemGroup['trabajo_nombre'] ?? '';
            ...
            @if($jobName)
                <div class="titulo-job" style="margin-top: 10px; margin-bottom: 3px;">
                    <strong>{{ $jobName }}@if($precioTrabajo) - ${{ $precioTrabajo }}@endif</strong>
                </div>
...
<br>
        ...
        <div class="payment-history-box" style="background: #f7f9fc; border: 1.5px solid #e3e8f0; border-radius: 10px; padding: 18px 22px; margin-bottom: 3mm; margin-top: 2mm;">
            <div class="section-title" style="margin-bottom: 14px; font-size: 14px; color: #2c3e50;">PAYMENT HISTORY</div>
            @if(count($registroV->pagos) > 0)
                <table class="pagos-table" style="margin-bottom: 12px;">
                    <thead>
                        <tr>
                            <th width="25%" style="padding-bottom: 6px;">Date</th>
                            <th width="35%" style="padding-bottom: 6px;">Method</th>
                            <th width="40%" style="text-align: right; padding-bottom: 6px;">Amount</th>
                        </tr>
                    </thead>
                    <tbody>
